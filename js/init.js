$(document).ready(function(){
    ...
    $("#transcriptShow").empty().html($("#transcriptText").html());
    ...
    if(wrapperPage)
    {
        $("body").append("<div id='couseLoader' style='position: fixed; width: 100%; height: 100%; z-index: 1000; background-color: rgba(0,0,0,0.5); top: 0'><img id='menuAudio' class='screenElem' src='./img/menuAudio.svg'><img id='seeMoreScreenBtn' class='screenElem' src='./img/seeMoreBtn.svg'><img id='prevNextScreenBtn' class='screenElem' src='./img/prevNextBtn.svg'><img id='prevNextScreenBtnSmall' class='screenElem' src='./img/prevNextBtnSmall.svg'><img id='slideIndicatorScreen' class='screenElem' src='./img/slideIndicator.svg'><img id='submitScreenBtn' class='screenElem' src='./img/submitScreenBtn.svg'><div style='width: 100%; text-align: center; position: relative; top: 40%'><p style='font-size: 2em; color: #fff;'>Course Loading...<br><span style=''>Please Wait</span></p></div></div>");
        setCoursePage();
    ...
    if(!wrapperPage)
    {
        ...
        audio.addEventListener("ended",function(){
            if(self != top)
            {
                ...
                if(parent.pages[parent.wrapperObj.currentPage].pageName == "page2")
                {
                    setTimeout(function(){
                        $('#go-to-page3').fadeIn();
                    }, 5000);
        ...
        $(".showVidModal").off("click").on("click",function(){
            ...
            {
                parent.$("#vidPlayer").empty().html('<div style="position: relative; display: block; max-width: 100%;"><div class="close-video-btn-wrap">Click this to Close <span class="close-video-btn glyphicon glyphicon-remove"></span></div><div style="padding-top: 56.25%;"><video data-video-id="' + $(this).data("video") + '" data-account="21478975001" data-player="SyzBB4N5Zg" data-embed="default" data-application-id class="video-js" controls style="position: absolute; top: 0px; right: 0px; bottom: 0px; left: 0px; width: 100%; height: 100%;"></video><script src="//players.brightcove.net/21478975001/SyzBB4N5Zg_default/index.min.js"></script></div></div>');
            }
    ...
    */
    ...
    function setQuizActive()
    {
        ...
        if(count >= 4)
        {
            ...
            $('#quizes').removeClass('inactive').addClass('active');
            $('#quizes').off().on('click', function(){
                parent.showCertainPage('quiz1');
            });
            $('#primary-text').hide();
            $('#secondary-text').show();
            parent.setCoursePage();
            parent.setBodyContainer();
        }
    }
    function playVideo(id, sectionID)
    {
        parent.$("#vidPlayer").empty().html('<div style="position: relative; display: block; max-width: 100%;"><div class="close-video-btn-wrap">Click to Close <span class="close-video-btn glyphicon glyphicon-remove"></span></div><div style="padding-top: 56.25%;"><video data-video-id="' + id + '" data-account="21478975001" data-player="SyzBB4N5Zg" data-embed="default" data-application-id class="video-js" controls style="position: absolute; top: 0px; right: 0px; bottom: 0px; left: 0px; width: 100%; height: 100%;"></video><script src="//players.brightcove.net/21478975001/SyzBB4N5Zg_default/index.min.js"></script></div></div>');
        parent.$("#vidModal").modal("show");
        parent.$("#pauseBtn").trigger("click");
        parent.$('.close-video-btn-wrap').off().on('click', function(){
            parent.$('#vidModal').modal('hide');
            parent.$("#vidPlayer video").eq(0)[0].pause();
        });
        
        setTimeout(function(){
            if($(('#' + sectionID)).hasClass('inactive'))
            {
                $(('#' + sectionID)).removeClass('inactive').addClass('active');
            }
            setQuizActive()
        }, 1000);
    }
    $('#shadow-it').off().on('click', function(){
        playVideo('6059554714001', 'shadow-it');
    });
    $('#iaas-paas').off().on('click', function(){
        playVideo('6059555826001', 'iaas-paas');
    });
    $('#business').off().on('click', function(){
        playVideo('6059550142001', 'business');
    });
    $('#collaboration').off().on('click', function(){
        setTimeout(function(){
            $('#collaboration').removeClass('inactive').addClass('active');
            setQuizActive();
        }, 1000);
        parent.showCertainPage('page7');
    });
    $('#close-page7-btn').off().on('click', function(){
        parent.showCertainPage('page3');
    });
    $('#go-to-page3-btn').off().on('click', function(){
        parent.showCertainPage('page3');
    });
});